...
export const searchitemSelector = createSelector(listSelector,searchSelector,selectorMenu,(listMenus,searchValue,selectorMenuItem) => {
  if (!listMenus) {
    return [];
  }
  ...
  return filteredMenus.filter((listMenu) =>
    listMenu.menus.filter((menu) => menu.name.includes(searchValue))
  );
})
export const searchCustomerSelector = createSelector(listCustomerSelector,searchSelector,(listCustomers,searchValue) => {
  if (!listCustomers) {
    return [];
  }
